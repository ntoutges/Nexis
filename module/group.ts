...
export class Group<T, Data=any> {
  private readonly groupsMap = new Map<T, number>(); // converts from group member to index in groups
  private readonly groups = new Map<number, T[]>(); // converts from index to group
  private readonly data = new Map<number, Data>();
  private readonly ids = new Ids();


  add(...elements: T[]) {
    const toCombine = new Set<number>()
    
    const groupData: T[] = []; // will be filled with elements that don't already exist in a group

    // check if element already exists in group
    // if so, note it so they can combine with current added group
    for (const element of elements) {
      if (this.groupsMap.has(element)) toCombine.add(this.groupsMap.get(element));
      else groupData.push(element);
    }
    ...
    // create new group
    for (const element of groupData) { this.groupsMap.set(element, id); }// add elements to groupsMap
  ...
  setGroupData(element: T, data: Data) {
    if (!this.groupsMap.has(element)) return; // element not in groups, so ignore
    const groupId = this.groupsMap.get(element);
    this.data.set(groupId, data);
  }

  getGroupData(element: T): Data {
    if (!this.groupsMap.has(element)) return null; // element not in groups, so ignore
    const groupId = this.groupsMap.get(element);
  ...
  private combine(indicesToCombine: number[]) {
    if (indicesToCombine.length < 2) return; // cannot combine 0/1 groups
  ...
  forEach(callback: (group: T[], data: Data, brk: () => void) => void) {
    let doBreak = false;
   ...
   */
  split(element: T, callback: (element: T) => boolean): [ originalGroup: T, newGroup: T ] {
    if (!this.groupsMap.has(element)) return; // element not in groups, so ignore
    const groupId = this.groupsMap.get(element);
    ...
    for (const item of group) {
      if (callback(item)) {
        ...
        if (newGroupEl === null) newGroupEl = item;
      }
      else if (originalGroupEl === null) originalGroupEl = item;
    }

    // check if all elements in group will stay together
    if (offshoots.length != 0 && offshoots.length != group.length) this.splitFromGroup(offshoots);
    return [ originalGroupEl, newGroupEl ];
  }

  // this function assumes all elements in offshoots are from the same group
  private splitFromGroup(offshoots: T[]) {
    if (offshoots.length == 0) return; // nothing to offshoot
    ...
    for (const offshoot of offshoots) {
      ...
      if (index == -1) continue; // just ignore
