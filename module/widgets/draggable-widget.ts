...
export class DraggableWidget extends Widget {
  ...
  constructor({
    ...
    if (options?.hideOnInactivity ?? false) this.container.classList.add("nexis-widgets-hide-on-inactive");
  ...
  protected dragInit() {
    if (this.doCursorDrag) this.container.classList.add("dragging");
    this.elListener.trigger("draginit", this.el);
    GlobalSingleUseWidget.unbuildType("contextmenu");
  }
  
  protected dragEnd() {
    if (this.doCursorDrag) this.container.classList.remove("dragging");
  ...
  private showButtons() {
    ...
    const title = this.header.querySelector<HTMLDivElement>(".nexis-draggable-widget-titles");
    if (title.classList.contains("show-buttons")) return; // already shown

    title.classList.add("show-buttons");

    const toPad = title.querySelector<HTMLDivElement>(".nexis-draggable-widget-button-holder").offsetWidth;
  ...
  setTitle(title: string) {
    const titleEl = this.header.querySelector(".nexis-draggable-widget-titles > .nexis-draggable-widget-title-texts") as HTMLElement;
